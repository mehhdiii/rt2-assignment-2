...
def action_client(action_name, action_msg, goal_msg, callbacks=None):
    ...
    def send_goal(arg):
        goal_msg_obj = goal_msg()
        widget_dict_to_msg(goal_msg_obj, widget_dict)
        if (last_sent_goal_handle is not None):
            last_sent_goal_handle(goal_msg_obj)
        a_client.send_goal(goal_msg_obj,
                           done_cb=done_handle,
